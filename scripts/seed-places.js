...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === '--bbox') {
      const v = argv[++i];
      const [south, west, north, east] = (v || '').split(',').map((x) => parseFloat(x.trim()));
...
function toRow(el) {
  ...
  const tags = el.tags ?? {};
  const display =
    tags.name || tags['operator'] || tags['brand'] || `${tags.shop || tags.amenity || 'Food resource'} (${el.type}/${el.id})`;
  const row = {
...
async function main() {
  ...
  if (!args.bbox && !args.center) {
    console.log('Provide --bbox south,west,north,east OR --center lat,lon --radiusKm N');
    process.exit(1);
  }
  const bbox = args.bbox || bboxFromCenter(args.center.lat, args.center.lon, args.radiusKm || 10);
  ...
  if (!supabaseUrl || !supabaseKey) {
    throw new Error('Missing EXPO_PUBLIC_SUPABASE_URL or EXPO_PUBLIC_SUPABASE_ANON_KEY/KEY in env');
  }
