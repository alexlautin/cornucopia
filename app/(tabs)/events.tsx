...
export default function EventsScreen() {
  ...
  const defaultEndDate = `${oneHourLater.getFullYear()}-${pad(
    oneHourLater.getMonth() + 1
  )}-${pad(oneHourLater.getDate())}`;
  const defaultEndTime = `${pad(oneHourLater.getHours())}:${pad(
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!isValidDate(startDate) || !isValidTime(startTime)) {
      Alert.alert(
        ...
        "Start date must be YYYY-MM-DD and time must be HH:MM (24h)."
      );
      return;
    }
    if (!isValidDate(endDate) || !isValidTime(endTime)) {
      Alert.alert(
        ...
        "End date must be YYYY-MM-DD and time must be HH:MM (24h)."
      );
      return;
    }
    ...
    try {
      ...
      console.log("Validating event hours", {
        startDate,
        startTime,
        endDate,
        endTime,
        ...
        >
          ...
          <View style={styles.card}>
            <View style={styles.field}>
              ...
              <TextInput
                ...
                placeholder="e.g. Community Pantry"
              />
            </View>
            ...
            <View style={styles.field}>
              ...
              <TextInput
                ...
                placeholder="e.g. Food distribution, Meeting"
              />
            </View>
          ...
          <View style={styles.eventsSection}>
            ...
            {loadingEvents ? (
              ...
              events.map((e) => {
                const addr =
                  e.address && typeof e.address === "object"
                    ? `${e.address.street ?? ""}${
                        e.address.address2 ? ", " + e.address.address2 : ""
                      }, ${e.address.city ?? ""}${
                        e.address.state ? ", " + e.address.state : ""
                      } ${e.address.postcode ?? ""}`.trim()
                    : e.address || "";
                const hours =
                  e.event_hours && typeof e.event_hours === "object"
                    ? Object.entries(e.event_hours)
                        .map(([d, v]: any) => `${d}: ${v.start}–${v.end}`)
                        .join(", ")
                    : "";
                ...
                return (
                  <View key={key} style={styles.eventCard}>
                    ...
                    {e.type ? (
                      <ThemedText style={styles.eventMeta}>{e.type}</ThemedText>
                    ) : null}
                    {addr ? (
                      <ThemedText style={styles.eventAddress}>
                        {addr}
                      </ThemedText>
                    ) : null}
                    {hours ? (
                      <ThemedText style={styles.eventMeta}>{hours}</ThemedText>
                    ) : null}
                  </View>
                );
              })
...
const styles = StyleSheet.create({
  ...
  field: { gap: 8 },
  label: { opacity: 0.8, fontSize: 14, fontWeight: "600" },
  input: {
